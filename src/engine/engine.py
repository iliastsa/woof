...
class Engine:
    ...
    def _render_record(self, record: Tuple[int, ...]) -> Tuple[str, ...]:
        return tuple(map(lambda y: self.id_to_const[y], record))
    ...
    def _output_rel(self, relation: str, output_dir: str):
        ...
        with open(os.path.join(output_dir, filename), 'w') as f:
            for record in self.true_facts[relation]:
                f.write(f'{", ".join(self._render_record(record))}\n')
    ...
    def print_all(self):
        ...
        for i, q_i in self.true_facts.items():
            if not i == '_U':
                print(f'{i}:', *(self._render_record(x) for x in q_i))
        ...
        for i, p_i in self.unknown_or_true_facts.items():
            if not i == '_U':
                print(f'{i}:', *(self._render_record(x) for x in p_i if not self.true_facts[i].member(x)))
